...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, vaddr_t temp = s->pc + 4; s->dnpc = (src1 + imm) & ~1; R(rd) = temp;);
  //slli
  ...
  //csrw
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrw   , I, word_t old; CSRRW(imm, src1, &old); R(rd) = old;);
  //csrrs
  ...
  // //fence
  // INSTPAT("??????? ????? ????? 000 ????? 00011 11", fence   , I, /* do nothing */);
  // //fence.tso
  // INSTPAT("0000000 ????? ????? 000 ????? 00011 11", fencetso, I, /* do nothing */);
  // //ecall

  // //pause
  // INSTPAT("??????? ????? ????? 000 ????? 11110 11", pause   , N, /* do nothing */);
  INSTPAT("??????? ????? ????? ??? ????? ????? ??", inv    , N, INV(s->pc);printf("invalid instruction: 0x%08x\n", s->isa.inst););
  INSTPAT_END();
